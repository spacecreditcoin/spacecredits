...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("SpaceCredits")
    updateDisplayUnit();
}
...
{
    QString ret = QString("SpaceCredits://%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SpaceCredits unit input in lnReqAmount, we generate the URI with SpaceCredits as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SPC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
