/*
 * SpaceCredits source code package
 *
...
/* SpaceCredits external data structures */

/*
 *****Context: structure to hold SpaceCredits inputs:
 ...
 ****************************
 ...
 You need to spend 1 GB of RAM and you run 5 passes of SpaceCreditsd with 4 parallel
 lanes.
...
/* same for SpaceCreditsd */
int hash_argon2d(void *out, size_t outlen, const void *in, size_t inlen,
                 const void *salt, size_t saltlen, unsigned int t_cost,
                 unsigned int m_cost);

/*
 * **************SpaceCreditsd: Version of SpaceCredits that picks memory blocks depending
 * on the password and salt. Only for side-channel-free
 * environment!!***************
 * @param  context  Pointer to current SpaceCredits context
 * @return  Zero if successful, a non zero error code otherwise
 */
int argon2d(argon2_context *context);

/*
 *  * **************Argon2i: Version of SpaceCredits that picks memory blocks
...
/*
 *   * **************Argon2ds: SpaceCreditsd hardened against GPU attacks, 20%
 * slower***************
 * @param  context  Pointer to current SpaceCredits context
 * @return  Zero if successful, a non zero error code otherwise
 */
...
/*
 * Verify if a given password is correct for SpaceCreditsd hashing
 * @param  context  Pointer to current SpaceCredits context
